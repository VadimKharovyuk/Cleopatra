...
//✅ Проверяет существование файла (не null, не пустой)
//✅ Проверяет размер файла (не превышает лимит)
//✅ Проверяет MIME-тип (разрешенные форматы)
//✅ Проверяет расширение файла
//✅ Проверяет имя файла (допустимые символы, длина)
//✅ Координирует процесс: решает нужна ли конвертация
@Component
@Slf4j
...
public class ImageValidator {
    ...
    // Обновленные множества с поддержкой HEIF
    ...
    /**
     * Валидирует и обрабатывает изображение (с конвертацией если нужно)
     */
    public ImageConverterService.ProcessedImage validateAndProcess(MultipartFile file) {
        log.debug("Начинаем валидацию и обработку изображения: {}",
        ...
        if (imageConverterService.needsConversion(file.getContentType())) {
            log.info("Файл {} требует конвертации из {}",
                    file.getOriginalFilename(), file.getContentType());
            return imageConverterService.convertToStandardFormat(file);
        }

        // Возвращаем без конвертации
        log.debug("Файл {} не требует конвертации", file.getOriginalFilename());
        return ImageConverterService.ProcessedImage.fromMultipartFile(file);
    }
